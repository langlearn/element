      ...
      <el-table-column
        ...
        <template scope="scope">
          ...
          <span @click="toggle(scope.$index)" v-if="!scope.row._leaf" class="is-parent">
            <i :class="scope.row._expanded?'el-icon-caret-bottom':'el-icon-caret-right'" style="padding-right: 4px;"></i>
...
<script type="text/babel">
  ...
  const treeToArray = function(me, data, parent, level, expandedAll) {
      let tmp = [];
      Array.from(data).forEach(record=> {
          if (record._expanded === undefined) {
              me.$set(record, '_expanded', expandedAll);
          }
          if (parent) {
              me.$set(record, '_parent', parent);
          }
          let _level = 0;
          if (level !== undefined && level !== null) {
              _level = level + 1;
          }
          me.$set(record, '_level', _level);
          tmp.push(record);
          if (record.children && record.children.length > 0) {
              me.$set(record, '_leaf', false);
              let children = treeToArray(me, record.children, record, _level, expandedAll);
              tmp = tmp.concat(children);
          } else {
              me.$set(record, '_leaf', true);
          }
      });
      return tmp;
  };
  ...
  export default {
    ...
    computed: {
      ...
      data() {
          let me = this;
          if (me.treeStructure) {
              let data = treeToArray(me, me.dataSource, null, null, me.treeExpandAll);
              return data;
          }
